  ...
  if (!result) {
    ...
    *result = my_get_zero_for_decimal();
    ...
    if (sign1 == MY_POSITIVE && sign2 == MY_POSITIVE) {
      ERROR_CODE = my_add_handle(value_1, value_2, &res);
    } else if (sign1 == MY_POSITIVE && sign2 == MY_NEGATIVE) {
      ERROR_CODE = my_sub(value_1, my_abs(value_2), &res);
    } else if (sign1 == MY_NEGATIVE && sign2 == MY_POSITIVE) {
      ERROR_CODE = my_sub(my_abs(value_1), value_2, &res);
      my_negate(res, &res);
    } else if (sign1 == MY_NEGATIVE && sign2 == MY_NEGATIVE) {
      ERROR_CODE = my_add_handle(my_abs(value_1), my_abs(value_2), &res);
      my_negate(res, &res);
    }
...
int my_add_handle(my_decimal value_1, my_decimal value_2,
  ...
  my_int256 res = my_binary_addition_int256(value_1l, value_2l);
